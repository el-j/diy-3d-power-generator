...
# ULTIMATIVES 20-POL UPGRADE (20 Oben, 20 Unten)
magnet_l = 20.0  # Länge
magnet_w = 5.0   # Breite
magnet_h = 3.0   # Dicke
anzahl_magnete = 20       
...
# 1. ROTOR (20 Rechteck-Magnete, Dreiecks-Markierungen & Ausrichtungs-Hilfen)
def make_rotor(is_top):
    ...
    # 5 Verschraubungslöcher! Verschoben auf 9 Grad Offset, damit sie PERFEKT zwischen den 20 Magneten liegen!
    for i in range(5):
    ...
    # ==========================================
    # SICHERHEITS-FEATURES ZUR MONTAGE (Für 20-Pol optimiert)
    # ==========================================
    # 1. Große Ausrichtungs-Kerbe (Notch) am Außenrand bei exakt 45 Grad! 
    # (Liegt passgenau in der Lücke zwischen Magnet 36° und Magnet 54°)
    ...
    # 2. Oben/Unten Indikator (Punkte an der Seite bei exakt 117 Grad!)
    # (Liegt passgenau in der Lücke zwischen Magnet 108° und Magnet 126°)
...
def make_backplate(is_top):
    ...
    # 5 Durchgangslöcher (9 Grad Offset!)
    for i in range(5):
        angle = math.radians(i * 72 + 9)
        x = rotor_schraub_r * math.cos(angle)
        y = rotor_schraub_r * math.sin(angle)
